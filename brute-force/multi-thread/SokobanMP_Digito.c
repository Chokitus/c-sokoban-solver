...
/*
	...
	----------              ----------
*/
void mergeLinkedLists(State** threadRoot, State** lastThreadState, State** mainRoot, State** mainLast){
...
#	pragma omp parallel num_threads(threads) shared(root, lastMainState, solution)
   {
      ...
      State *s;
		...
		//Variável de condição que nos diz se devemos pegar um estado da lista principal ou não
      ...
      while(!(*solution)){
         ...
         //Se a variável de condição foi 1, devemos pegar um estado da lista principal.
			...
			//caso esta seja a primeira iteração de cada thread
         if(popMainList){
...
#				pragma omp critical(popMerge)
         ...
         for(int i = 0; i < 4 && !(*solution); i++){
            ...
            if(movePlayer(s, i) != 0){
               ...
               if(insertState(threadRoot, s, lastThreadState)){
                  //Entrou aqui quer dizer que o estado era final, de acordo com a definição de estado final.
                  printPath(s);
						*solution = 1;
               }
            }
         ...
         //Tentaremos criar uma lista de pelo menos SIZE_THREAD_LIST elementos antes de adicionar à lista principal.
			//Caso não conseguimos estados suficientes, activeThreadStates = -1, todos os nós que pegamos eram inúteis.
			//Isso significa que precisamos pegar novos nós da lista principal
         if(activeThreadStates < SIZE_THREAD_LIST && activeThreadStates > 0){
            //Desempilhamos mais um, agora da nossa lista temporária, pois não passamos da quantidade necessária
            ...
            if(activeThreadStates > 0 && !(*solution)){
               //há lista para empilhar
					//Como no pop acima, esta região é critica (e de mesmo nome do pop) pois mexe com a lista principal
#					pragma omp critical(popMerge)
               mergeLinkedLists(&threadRoot, lastThreadState, &root, lastMainState);

					//Não há mais estados ativos no thread
               activeThreadStates = 0;
            }/*if*/
					//Ordenamos que se retire da lista principal mais um nó para ser expandido
					popMainList = 1;
         }/*else*/
